 ...
 * 드롭다운에서 전체 탭을 골라 열 수 있다. 순서는 드래그로 바꿀 수 있다(사이드바와 같은
 * HTML5 drag&drop 방식이며, 커서가 대상 탭의 좌/우 절반 중 어디인지로 삽입 위치를 정한다).
 */
            ...
            return (
              <div
                ...
                onDragOver={(e) => {
                  ...
                  // 커서가 대상 탭의 왼쪽 절반이면 앞, 오른쪽 절반이면 뒤에 넣는다.
                  const rect = e.currentTarget.getBoundingClientRect()
                  const before = e.clientX < rect.left + rect.width / 2
                  setDropAt((prev) =>
                    prev?.id === tab.id && prev.before === before
                      ? prev
                      : { id: tab.id, before }
                  )
                }}
                onDrop={(e) => {
                  ...
                  e.preventDefault()
                  const rect = e.currentTarget.getBoundingClientRect()
                  onMove(dragId, tab.id, e.clientX < rect.left + rect.width / 2)
                  endDrag()
                }}
                onDragEnd={endDrag}
                className={cn(
